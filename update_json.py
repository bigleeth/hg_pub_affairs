...
def collect_bill_info(bill_name):
    ...
    if table:
        ...
        for row in rows:
            ...
            if len(cols) >= 8:
                bill_id = cols[1].find('a')['onclick'].split("'")[1] if cols[1].find('a') else ''
                content_id = cols[6].find('a')['onclick'].split("'")[1] if cols[6].find('a') else ''
                bills.append({
                    '의안번호': cols[0].text.strip(),
                    '의안명': {'text': cols[1].text.strip(), 'link': f"javascript:fGoDetail('{bill_id}', 'billSimpleSearch')"},
                    '제안자구분': cols[2].text.strip(),
                    '제안일자': cols[3].text.strip(),
                    '의결일자': cols[4].text.strip(),
                    '의결결과': cols[5].text.strip(),
                    '주요내용': {'text': '주요내용 보기', 'link': f"javascript:ajaxShowListSummaryLayerPopup('{content_id}')" },
